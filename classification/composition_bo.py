...
class AbstractCompositionBayesianOptimization(ABC):
    def __init__(self,
        ...
        _all_elem_features = pd.read_excel(os_path('data\\ele_vec.xlsx'))
    ...
    def feature_selection(self):
        ...
        fs = FeatureSelectionGA(env, verbose = 1)
        fs.generate(
            n_pop = 192, 
            cxpb = 0.8, 
            mutxpb = 0.1, 
        ...
        best_ind = fs.get_fitest_ind()

        # buffer GA results
        self._sel_elem_feat_idxs = best_ind.f_idx_list
    ...
    def ei_acqf(self, mean, std, xi = 0.0):
        ...
        z = a / std
        ei_vals[mask] = a * self._norm_dist.cdf(z) + std * self._norm_dist.log_prob(z).exp()
        return ei_vals

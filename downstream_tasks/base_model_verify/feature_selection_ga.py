...
def get_xgb() -> XGBRegressor:
    """
    Get XGBRegressor object with optimized parameters
    """
    return XGBRegressor(
        objective="reg:squarederror",
        max_depth=10,  # 向下取整 from 16
        learning_rate=0.15,  # from 0.155
        n_estimators=242,  # 向下取整 from 242.46
        min_child_weight=8,  # 向上取整 from 7.72
        subsample=0.87,  # from 0.87
        colsample_bytree=0.85,  # from 0.85
        gamma=11.68,  # from 11.68
        reg_alpha=0.49,  # from 0.49
        reg_lambda=1.53,  # from 1.53
        random_state=42,
    )
...
def get_svr() -> SVR:
    """
    Get SVR object with optimized parameters
    """
    return SVR(
        kernel="rbf",
        C=600,  # from 600.1203196939409
        epsilon=0.9,  # from 0.8934082678382989
        gamma=0.03,  # from 0.026030306218743252
        cache_size=1000,  # 增加缓存大小以加快计算
    )


def get_mlp() -> MLPRegressor:
    return MLPRegressor(
        hidden_layer_sizes=(1024, 128, 512, 256, 512),  # 实际的隐藏层节点数
        activation="relu",
        solver="adam",
        learning_rate="adaptive",
        learning_rate_init=0.0003639,
        alpha=0.005114,
        batch_size=16,
        max_iter=3000,
        early_stopping=True,
...
class Env:
    ...
    def calculate_fitness(self, ind: Individual):
        ...
        assert len(feature_idx_list) == self.f_n
        assert len(set(feature_idx_list)) == self.f_n
...
class FeatureSelectionGA:
    ...
    def par_eval(self, pop: List[Individual]) -> List[float]:
        ...
        :return: fitnesses
        """
        fitnesses = joblib.Parallel(n_jobs=THREAD_N)(
            joblib.delayed(self.env.calculate_fitness)(x) for x in pop
        )
        return fitnesses

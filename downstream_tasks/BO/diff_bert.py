...
def load_data(prop_id: int, bt_feat_idx: int):
    ...
    bt_feat_files = [
        'data/ele_vec.xlsx',
        'data/BERTBase_ele_vec.xlsx',
        'data/deberta-v3-xsmall_ele_vec.xlsx',
        'data/scibert_ele_vec.xlsx',
        'data/matscibert_ele_vec.xlsx',
    ]

    bt_feat_file_path = bt_feat_files[bt_feat_idx]

    elem_feature_bt = pd.read_excel(bt_feat_file_path)   # NOTE: bert embedding features
...
if __name__ == '__main__':
    ...
    traj_id = int(sys.argv[3])  # bert embedding feature index

    for bt_feat_idx in range(5):
        set_seed(traj_id)
        ''' 1. 准备数据 '''
        comp_data, _, prop_data, elem_feature_ep, elem_feature_bt = load_data(prop_id, bt_feat_idx)     # ~ 158 data points
        ...
        joblib.dump(
            res_buff_bt,
            f'r2-{prop_id}-{f_n}-{bt_feat_idx}-{traj_id}-{str(uuid.uuid4())[:8]}.pkl'
        )
...
def get_random_forest() -> RandomForestRegressor:
    ...
    return RandomForestRegressor(
        n_estimators=100,  # NOTE: 20240817_val = 200
        random_state=43,
    )
...
def get_gpr() -> GaussianProcessRegressor:
    ...
    """
    fit_init, fit_bounds = 10.0, (1e-5, 1e8)
    gpr_kernel = ConstantKernel(fit_init, fit_bounds) * Matern(
        fit_init, fit_bounds
    ...
    model = GaussianProcessRegressor(
        kernel=gpr_kernel, n_restarts_optimizer=100, random_state=42
    )
    return model
'''
...
def get_random_forest() -> RandomForestClassifier:
    """
    获取RandomForestClassifier对象，用于分类

    :return: RandomForestClassifier对象
    """
...
def get_gpc() -> GaussianProcessClassifier:
    """
    获取GaussianProcessClassifier对象，用于分类
    """
    fit_init, fit_bounds = 10.0, (1e-5, 1e8)
    gpr_kernel = ConstantKernel(fit_init, fit_bounds) * Matern(fit_init, fit_bounds)
...
'''
def custom_cross_validation(model, x, y) -> float:
    """
    ...
    for _ in range(CV_N_REPEAT):
        ...
        r2_buff.append(r2_score(y_test_buff, y_pred_buff))

    return np.mean(r2_buff)
...
def custom_cross_validation(model, x, y) -> float:
    """
    Custom 10-fold cross validation for classification.
    ...
    for _ in range(CV_N_REPEAT):
        ...
        # 使用accuracy_score替代r2_score
        acc_buff.append(accuracy_score(y_test_buff, y_pred_buff))
        # 也可以使用F1分数：
        # acc_buff.append(f1_score(y_test_buff, y_pred_buff, average='weighted'))

    return np.mean(acc_buff)


def calculate_fitness(model, feature_val, prop_val, feature_num, scaler, ind):
    ...
    x = scaler.fit_transform(x)
    fitness = custom_cross_validation(model, x, prop_val)
...
class Env:
    ...
    def __init__(self, wavg_feature, props, f_n: int = 4) -> None:
...
class Individual:
    ...
    def key(self) -> str:
        return "*".join(map(str, sorted(self.f_idx_list.tolist())))
...
def mutate(parent: Individual, mutation_prob: float, total_f_num: int) -> Individual:
    ...
    mutate_val = random.sample(
        list(set(range(total_f_num)) - set(f_idx_list)), len(mutate_pos)
    )
...
def cross_over(
    parent_1: Individual, parent_2: Individual, crx_pb: float = 0.25
    ...
    for i in range(len(only_in_1)):
        if random.random() < crx_pb:
            new_f_idx_list_1[i], new_f_idx_list_2[i] = (
                new_f_idx_list_2[i],
                new_f_idx_list_1[i],
            )
...
class FeatureSelectionGA:
    ...
    def __init__(self, ff_obj: Env, verbose: int = 0):
        ...
        if self.verbose == 1:
            print(
                "Will select best features among {} features.".format(
                    self.env.feature_val.shape[1]
                )
            )
            print(
                "Shape of train_x: {} and target: {}".format(
                    self.env.feature_val.shape, self.env.prop_val.shape
                )
            )
    ...
    def par_eval(self, pop: List[Individual]) -> List[float]:
        ...
        for ind in pop:
            key = ind.key()
            if key not in self.fitness_dict:
                uneval_ind_list.append(ind)

        # evaluate unevaluated individuals
        par_fitnesses = joblib.Parallel(n_jobs=THREAD_N)(
            joblib.delayed(calculate_fitness)(
                self.env.model,
            ...
            for ind in uneval_ind_list
        )
        # update fitness_dict
        for ind, fitness in zip(uneval_ind_list, par_fitnesses):
            self.fitness_dict[ind.key()] = fitness
        # return fitnesses of all individuals
        return [self.fitness_dict[ind.key()] for ind in pop]

        # fitnesses = []
        # for ind in pop:
        #     key = ind.key()
        #     if key not in self.fitness_dict:
        #         self.fitness_dict[key] = calculate_fitness(
        #             self.env.model,
        #             self.env.feature_val,
        #             self.env.prop_val,
        #             self.env.feature_num,
        ...
        #             ind
        #         )
        #     fitnesses.append(self.fitness_dict[key])
        # return fitnesses
    ...
    def generate(self, n_pop, cxpb=0.5, mutxpb=0.2, ngen=5):
        ...
        pop = [init_individual(self.env.total_f_N, self.env.f_n) for _ in range(n_pop)]
        ...
        for g in range(ngen):
            ...
            for ind_1, ind_2 in zip(
                selected_pop[::2], selected_pop[1::2]
            ):  # TODO: check cross_over order
        ...
        print("Best individual is %s, %s" % (self.best_ind, self.best_ind.fitness))

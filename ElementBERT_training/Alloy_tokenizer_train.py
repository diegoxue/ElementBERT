...
old_tokenizer = AutoTokenizer.from_pretrained(
    "/hy-tmp/deberta-v3-xsmall/models--microsoft--deberta-v3-xsmall")


tokenizer = old_tokenizer.train_new_from_iterator(
    text_iterator=training_corpus, vocab_size=128_100
)
...
def full_sent_tokenize(file_name):
    ...
    tok_sents = [
        tokenizer(s, padding=False, truncation=False)["input_ids"] for s in tqdm(sents)
    ]
    for s in tok_sents:
        s.pop(0)
...
tokenized_datasets = DatasetDict(
    {"train": Dataset.from_pandas(df_train), "val": Dataset.from_pandas(df_val)}
)
